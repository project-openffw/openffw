...
if exist(options.template,'dir') ~= 7
...
%-------------------------------------------------------------------------------
if ~options.load
	mdirs = cell(1,length(mfiles));
	names = cell(1,length(mfiles));
	for i=1:length(mfiles)
		[mdirs{i}, names{i}] = fileparts(mfiles{i});
		if isempty(mdirs{i}), mdirs{i} = '.'; end
	end

    mdir = getDir(options.mFiles,{},options.recursive);
...
%-------------------------------------------------------------------------------
for i=1:length(mdir)
	if exist(fullfile(options.htmlDir,mdir{i}),'dir') ~= 7
		ldir = splitpath(mdir{i});
		for j=1:length(ldir)
			if exist(fullfile(options.htmlDir,ldir{1:j}),'dir') ~= 7
...
for i=1:length(d)
	...
	if ~strcmp(ext,'.tpl') % do not copy .tpl files
		if ~(exist(fullfile(options.htmlDir,d{i}),'file'))
			if options.verbose
...
tpl = set(tpl,'block','TPL_MFILE','input','theinput');
tpl = set(tpl,'block','TPL_MFILE','output','theoutput');
tpl = set(tpl,'block','TPL_MFILE','description','thedescription');
tpl = set(tpl,'var','DATE',[datestr(now,8) ' ' datestr(now,1) ' ' ...
							datestr(now,13)]);
                        
tpl = set(tpl,'block','TPL_MFILE','anycrossrefcall','theanycrossrefcall');
tpl = set(tpl,'block','TPL_MFILE','anycrossrefcalled','theanycrossrefcalled');
tpl = set(tpl,'block','TPL_MFILE','crossreference','thecrossreference');
...
for i=1:length(mdir)
	for j=1:length(mdirs)
		if strcmp(mdirs{j},mdir{i})
            ...
            if ~isempty(descr)
                tpl = set(tpl,'var','DESCRIPTION',horztab(descr,options.tabs));
                tpl = parse(tpl,'thedescription','description');
            else
                tpl = set(tpl,'var','thedescription','');
            end
            
            %- Set cross-references template fields:
            %  Function called
            if isempty(find(hrefs(j,:) == 1)) && isempty(find(hrefs(:,j) == 1))
                tpl = set(tpl,'var','thecrossreference','');
            else
                tpl = parse(tpl,'thecrossreference','crossreference');
                ind = find(hrefs(j,:) == 1);
                if isempty(ind)
                    tpl = set(tpl,'var','theanycrossrefcall','');
                else
                    tpl = parse(tpl,'theanycrossrefcall','anycrossrefcall');
                ...
                ind = find(hrefs(:,j) == 1);
                if isempty(ind)
                    tpl = set(tpl,'var','theanycrossrefcalled','');
                else
                    tpl = parse(tpl,'theanycrossrefcalled','anycrossrefcalled');
            ...
            if options.input
                ...
                if ischar(tline)
                    tline = fgetl(fid2);
                    while ischar(tline) && ...
                            ( ((length(tline) > 1) && isempty(findstr('%%',tline))) || ...
                            ( (length(tline) == 1) && ~strcmp(tline,'%') ) || isempty(tline) )   
					    [myline,nbsubroutine] = formatCodeLine(j,tline,nbsubroutine,subname,...
                                        indhrefnames,hrefnames,options,mdirs,subroutine,synopsis);
                        input = [input sprintf('%s\n',myline)];
                        tline = fgetl(fid2);
                    end
                end
                if ~isempty(input)
                    tpl = set(tpl,'var','INPUT',horztab(input,options.tabs));
                    tpl = parse(tpl,'theinput','input');
                else
                    tpl = set(tpl,'var','theinput','');
                end
            else
                tpl = set(tpl,'var','theinput','');
            end
            ...
            if options.output
                ...
                if ~isempty(output)
                    tpl = set(tpl,'var','OUTPUT',horztab(output,options.tabs));
                    tpl = parse(tpl,'theoutput','output');
                else
                    tpl = set(tpl,'var','theoutput','');
                end
            else
                tpl = set(tpl,'var','theoutput','');
            end
...
%==========================================================================
function [myline,nbsubroutine] = formatCodeLine(j,tline,nbsubroutine,...
          subname,indhrefnames,hrefnames,options,mdirs,subroutine,synopsis)
...
function mfiles = getmfiles(mdirs,mfiles,recursive)
	...
	for i=1:length(mdirs)
		...
		elseif exist(mdirs{i},'dir') == 7 % Directory
			w = what(mdirs{i});
...
%===============================================================================
function dirs = getDir(mdirs,dirs,recursive)
...
for i=1:length(mdirs)
    if exist(mdirs{i},'dir') == 7 % Directory
        ...
        if recursive
            ...
            d = {d{~ismember(d,{'.' '..'})}};
            for j=1:length(d)
                dirs = getDir(cellstr(fullfile(mdirs{i},d{j})),...
                    dirs,recursive);
            end
        end
    else
        fprintf('Warning: Unprocessed dir %s.\n',mdirs{i});
    end
end
%===============================================================================
