...
evalGrad = reshape(evalGrad',[1 2 length(points(:,1))]);
...
if norm(absGrad) > 1e-14
    term1 = matMul(reshape(evalD2W./absGrad.^2,[1 1 length(points(:,1))]),XYXZ);
    term2 = -matMul(reshape(evalDW./absGrad.^2,[1 1 length(points(:,1))]),XYXZ);
else
    term1 = 0;
    term2 = 0;
end
