function p = TWDWRgetLambda1Funcs(p)
% author: Lena Noack
...
function divLambda1 = getDivLambda1_h(x,y,curElem,lvl,p)

divStressBasis = p.statics.divStressBasis;
lambda14e = p.level(lvl).grad4e;

evalBasis = divStressBasis(x,y,curElem,lvl,p);

divLambda1 = (lambda14e(curElem,:)*evalBasis)';

%% supply Alambda1
function Alambda1 = getAlambda1_h(x,y,curElem,lvl,p)
...
%% supply divAlambda_1
function divAlambda1 = getDivAlambda1(x,y,curElem,lvl,p)
...
divAlambda1 = divAlambda1*ones(length(x),1);

%% supply I^2(\lambda1_h)
function I2lambda1 = getI2lambda1_h(x,y,curElem,lvl,p)
...
evalAlambda1 = Alambda1(coords(:,1),coords(:,2),curElem,lvl,p);
...
function gradI2lambda1 = getGradI2lambda1_h(x,y,curElem,lvl,p)
...
evalAlambda1 = repmat(evalAlambda1',[1 1 length(x)]);

gradI2lambda1 = matMul(evalAlambda1,evalGradBasisP2);

%% supply div I^2(\lambda1_h)
function divI2lambda1_h = getDivI2lambda1_h(x,y,curElem,lvl,p)

gradI2lambda1_h = p.statics.gradI2lambda1_h;

evalGradI2lambda1h = gradI2lambda1_h(x,y,curElem,lvl,p);
