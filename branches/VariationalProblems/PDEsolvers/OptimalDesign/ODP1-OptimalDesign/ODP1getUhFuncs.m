...
p.statics.Aw_h = @getAw_h;
p.statics.AGrad_wh = @getAGrad_wh;
...
%% supply average w_h: A(w_h)
function Auh = getAw_h(x,y,curElem,lvl,p)

Auh4n = p.level(lvl).Awh;
n4e = p.level(lvl).geom.n4e;
basisU = p.statics.basisU;
basisP1 = basisU(x,y,curElem,lvl,p)';
...
%% supply average grad w_h: A(grad w_h)
function Aph = getAGrad_wh(x,y,curElem,lvl,p)

Aph4n = p.level(lvl).AGrad_wh;
n4e = p.level(lvl).geom.n4e;
basisU = p.statics.basisU;
basisP1 = basisU(x,y,curElem,lvl,p)';
