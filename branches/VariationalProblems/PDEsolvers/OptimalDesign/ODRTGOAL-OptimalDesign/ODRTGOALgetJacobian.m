...
% evaluation of ansatz-function, dim = length(x)\times 2
% evalFunc = func(x,y,curElem,lvl,p);
% evaluation of the test-functions, 
% dim = squeeze([nrTestFunctions,2,length(x)])
% evalTestFunc1 = testFunc1(x,y,curElem,lvl,p);
% evalTestFunc2 = testFunc2(x,y,curElem,lvl,p);
% evalTestFunc3 = testFunc3(x,y,curElem,lvl,p);

% absFunc = (evalFunc(:,1).^2 + evalFunc(:,2).^2).^(1/2);

% evaluaton of the first nonLinear functions. dim = length(x)
...
%     evalTestFunc1 = reshape(evalTestFunc1',[1 2 length(x)]);
% end
% sizeTestFunc2 = size(evalTestFunc2);
% if length(sizeTestFunc2) == 2
%     evalTestFunc2 = reshape(evalTestFunc2',[1 2 length(x)]);
% end
% sizeTestFunc3 = size(evalTestFunc3);
% if length(sizeTestFunc3) == 2
%     evalTestFunc3 = reshape(evalTestFunc3',[1 2 length(x)]);
% end

% evalFunc = reshape(evalFunc',[1 2 length(x)]);
...
if norm(absSigma) > 0
    ...
    term1 = matMul(reshape(-2*evalD2W./absSigma.^3,[1 1 length(x)]),XWXZXY);
    % W''(|x|)/|x|^2*( (x*y)(w*z)+(x*z)(w*y) )
    term2 = matMul(reshape(evalD2W./absSigma.^2,[1 1 length(x)]),XYZW+XZYW);
    % W''(|x|)/|x|^2*(x*w)(y*z)
    term3 = matMul(reshape(evalD2W./absSigma.^2,[1 1 length(x)]),XWYZ);
    % - W''(|x|)/|x|^4*(x*z)(x*y)(x*w)
    term4 = matMul(reshape(-evalD2W./absSigma.^4,[1 1 length(x)]),XWXZXY);
    % -3*W'(|x|)/|x|^2*(x*w)(y*z)
    term5 = matMul(reshape(-3*evalDW./absSigma,[1 1 length(x)]),XWYZ);
    % 3*W'(|x|)/|x|^4*(x*w)(x*z)(x*y)
    term6 = matMul(reshape(3*evalDW./absSigma.^3,[1 1 length(x)]),XWXZXY);
    % W'(|x|)/|x|^3*(2*(y*z)(x*w)-(w*z)(x*y)-(x*z)(w*y))
    term7 = matMul(reshape(evalDW./absSigma.^2,[1 1 length(x)]),2*XWYZ-XYZW-XZYW);
