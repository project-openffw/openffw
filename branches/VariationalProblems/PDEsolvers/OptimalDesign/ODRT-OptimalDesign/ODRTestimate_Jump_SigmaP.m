function p = ODRTestimate_Jump_SigmaP(p)
%author: Lena Noack
...
for curElem = 1:nrElems
    evalGrad(curElem,:) = grad_h(midPoint4e(curElem,1),midPoint4e(curElem,2),...
                                    curElem,lvl,p);
end
...
jumpS4ed = length4ed.*integrate(n4ed,curLvl,degree,@integrand,p); %h_E ||[sigma_h]nu||^2_{L^2}

eta4ed = sqrt(sqrt(jumpS4ed).*sqrt(jumpP4ed)); % (h_E^{3/2} ||[sigma_h]nu||_{L^2} |[p_h]nu| )^{1/2}
%eta4ed = sqrt(jumpP4ed); % (h_E^{3/2} ||[sigma_h]nu||_{L^2} |[p_h]nu| )^{1/2}
