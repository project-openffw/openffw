function p = bisection(p)
% bisection

% Copyright 2007 Jan Reininghaus, Robert Huth
%
...
%refineElemsBisec5 = loadField('p.level(end)','refineElemsBisec5',p,false(nrElems,1));
...
% Green refinement with refenrence edge (first edge) marked
nrGreenElems = 0;
I = find(nrMarkedEd4MarkedElems == 1 & markedEdges( ed4e(refineElems,1) )');
if ~isempty(I)
...
% Green refinement with second edge marked
I = find(nrMarkedEd4MarkedElems == 1 & markedEdges( ed4e(refineElems,2) )');
if ~isempty(I)
	gElems = refineElems(I);
	% todo
	[dontUse,dontUse,newN] = find( newNode4e(gElems,:)' );
	newGreenElems = [n4e(gElems,[3 1]) newN;...
                     n4e(gElems,[1 2]) newN];
	newn4e = [newn4e;newGreenElems];
    nrGreenElems = size(newGreenElems,1);
    dummy = [gElems;gElems];
...
% Green refinement with third edge marked
I = find(nrMarkedEd4MarkedElems == 1 & markedEdges( ed4e(refineElems,3) )');
if ~isempty(I)
	gElems = refineElems(I);
	% todo
	[dontUse,dontUse,newN] = find( newNode4e(gElems,:)' );
...
if ~isempty(I)
	...
	% BlueLeft (first & third edge) refinement
	bl = find(RefineEdge(1,:) == 1 & RefineEdge(2,:) == 3);
	if ~isempty(bl)
	...
	% BlueRight (first and second) refinement
	br = find(RefineEdge(1,:) == 1 & RefineEdge(2,:) == 2);
	if ~isempty(br)
	...
	% BlueRight (second and third) refinement
	br = find(RefineEdge(1,:) == 2 & RefineEdge(2,:) == 3);
	if ~isempty(br)
		newBlueRight = [ newN(br,1), n4e(bElems(br),3), newN(br,2);...
                        n4e(bElems(br),1), newN(br,1), newN(br,2);...
                        n4e(bElems(br),1), n4e(bElems(br),2), newN(br,1)];
		newn4e = [newn4e; newBlueRight];
        nrBlueRightElems = size(newBlueRight,1);
        dummy = [bElems(br);bElems(br);bElems(br)];
        parents4e( size(newn4e,1)+1 : size(newn4e,1)+nrBlueRightElems ) = dummy;
