...
%% OUTPUT
p.level(end).etaT = nu;
%p.level(end).estimatedError = abs(sum(nu));%norm(nu,2);        %est. for ||sigma-sigma_h||^2_L^{4/3}
p.level(end).estimatedError = (abs(sum(nu))).^0.5;%norm(nu,2);  %est. for ||sigma-sigma_h||_L^{4/3}
