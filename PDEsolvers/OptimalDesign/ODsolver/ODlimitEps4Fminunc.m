function p = ODlimitEps4Fminunc(p)
% author: David Guenther 
% compute solution for a sequence of theta_k
...
epsSequence = p.params.epsSequence;
...
while(nrDoF < maxNrDoF && curLevel <= maxLevel)
    ...
    while k <= length(epsSequence)
        p.problem.epsilon = epsSequence(k);
        epsilon = epsSequence(k)
        freeNodes = p.level(end).enum.freeNodes;

        x0 = p.level(end).x;
        p.level(end).x0 = x0;
        % find x s.t. E(x) = 0 with E given in getFuncVal.m
%         [x,fval,exitflag,output,grad,hessian] = fminunc(getNL4fminunc,x0(freeNodes),options,p);
...
%         p.level(end).hessian = hessian;
    ...
    fprintf('\n Level = %d \t Epsilon = %.2g \t Error = %.2g \t DoF = %d\n',curLevel+1,p.problem.epsilon,estimatedError,nrDoF);
end
