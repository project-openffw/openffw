...
%primalResiduum = sqrt( nu_T ) + sqrt( 1/4*1./h_T.*sum(nu_E(ed4e),2) );

dnu_T = integrate(n4e,curLvl,4*degree,@InterpolationElems,p);
dnu_E = integrate(n4ed,curLvl,4*degree,@InterpolationEdges,p);
dualWeight = sqrt( dnu_T + h_T.*sum(dnu_E(ed4e),2) );
%dualWeight = sqrt( dnu_T ) + sqrt( h_T.*sum(dnu_E(ed4e),2) );
...
function val = InterpolationElems(x,y,curElem,lvl,p)
Au_h = p.statics.I2u_h;
u_h = p.statics.u_h;

%% Residuum
Auh = Au_h(x,y,curElem,lvl,p);
...
function val = InterpolationEdges(x,y,curEdge,lvl,p)
e4ed = p.level(lvl).enum.e4ed;
Au_h = p.statics.I2u_h;
u_h = p.statics.u_h;

curElem = e4ed(curEdge,:);

%% Residuum
Auh = Au_h(x,y,curElem(1),lvl,p);
uh = u_h(x,y,curElem(1),lvl,p);

if curElem(2) ~= 0
    Auh = Auh+Au_h(x,y,curElem(2),lvl,p);
    uh  = uh +u_h(x,y,curElem(2),lvl,p);
end
