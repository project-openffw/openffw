function p = ODRTestimateCompareSols(p)
...
%% OUTPUT
p.level(end).etaT = nu;
p.level(end).estimatedError = sqrt(abs(sum(estErr))); %est. for ||sigma-sigma_h||
%p.level(end).estimatedError = abs(sum(estErr));       %est. for ||sigma-sigma_h||^2
%p.level(end).estimatedError = sum(nu);
%p.level(end).estimatedError = norm(nu,2);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val = integrand(x,y,parts,lvl,p)
% RT Aufrufen, siehe alt/ODRTestimate_P1
%sigma_h = permute(p.statics.sigma_h(x,y,parts,lvl,p),[3 2 1]);
grad_h = p.statics.grad_h;
DW = p.problem.nonLinearExactDer;                       %DW(Du)/|Du|
evalGrad = grad_h(x,y,parts,lvl,p);
absGrad = ( evalGrad(:,1).^2 + evalGrad(:,2).^2 ).^(1/2);
sigma_h = DW(absGrad,parts,lvl,p).*absGrad;

q = -[x./2 y./2];
val(1,1,:) = (sigma_h-q(:,1)).^2+(sigma_h-q(:,2)).^2;

% Averaging of dual Solution: ADv_h-Dv_h
function val = Average(x,y,parts,lvl,p)
Ap_h = p.statics.gradI2u_h;%Agradh;
p_h = p.statics.grad_h;%GradU_h;
