function p = P2Interpolation(p,curLvl)
...
%    fprintf('- Start Binsearch ')
%    [a,b] = binsearch(J,curNode);
%    fprintf('- End Binsearch ')
%    patch = S(a:b);
    [dontUse,dontUse,neighbours] = find(e4n(curNode,:));
    patch = neighbours;
    edges = ed4e(patch,:);
...
%fprintf('- End Interpolation')

%% Output
p.level(curLvl).DWRP2u4e = u(dofU4e);
